jQuery(document).ready(function($) {
	...
	// New map type select
	$('#blockmap-new-type').on('change', function() {
		if (this.value != 'custom') $('#blockmap-mapfile').hide();
		else $('#blockmap-mapfile').show();
	});

	// Edit mode
	$('#blockmap-editmode').click(function() {
		$('.blockmap-rawedit').toggle();
		$('#blockmap-admin-map').toggle();
		$(this).val(function(i, text) { return text === blockmap_localization.raw ? blockmap_localization.map : blockmap_localization.raw; });
	});

	// Indentation
	$('#blockmap-indent').change(function() {
		var ischecked = $(this).is(':checked'),
			object = JSON.parse($('#blockmap-mapdata').val());
		if (ischecked) $('#blockmap-mapdata').val(JSON.stringify(object, null, 4));
		else $('#blockmap-mapdata').val(JSON.stringify(object));
	});

	// Import select
	$("#blockmap-new-type").change(function() {
		if ($(this).val() == 'import') $('#blockmap-import').show();
		else $('#blockmap-import').hide();
	});

	// WordPress colorpicker
	$('.blockmap-color-picker').each(function() {
		var text = $(this).attr('data-text'),
			cp = $(this).wpColorPicker();
		if (text) cp.parent().parent().prev().find('.wp-color-result-text').html(text);
	});

	$('.blockmap-alpha-color-picker').each(function() {
		var text = $(this).attr('data-text'),
	...
	$('#pins-input > li').click(function() {
		...
		if ($('.blockmap-pin', this).hasClass('pin-label')) $('#landmark-settings .label-input').show();
		...
		if (selected.length) {
			...
			var data = selected.data('landmarkData'),
				pin = $('.blockmap-pin', this).data('pin');

			selected.attr('class', 'blockmap-pin selected-pin ' + pin);
			data.pin = pin;
		}
	});
	...
	function BlockMapAdmin() {
		...
		this.newLocation = function(id) {
			...
			$('#landmark-settings .blockmap-landmark-field').val('');
			...
			// Change button text
			$('.save-landmark').val(blockmap_localization.add);
			$('.duplicate-landmark').hide();
		}
	}
	var admin = new BlockMapAdmin().init();
	...
	$('.save-landmark').click(function() {
		...
		if (!$('#landmark-settings .id-input').val()) {
			alert(blockmap_localization.missing_id);
			return false;
		}
		...
		if (selected.length) {
			...
			newLandmark(data);
			$(this).val(blockmap_localization.save);
		}
	});
	...
	var newLandmark = function(data) {
		...
		var pin = $('<a></a>').attr({'href': '#' + data.id, 'title': data.title}).addClass('blockmap-pin selected-pin').addClass(data.pin).css({'top': '50%', 'left': '50%'}).click(function(e) {
			e.preventDefault();
		}).appendTo($('.blockmap-layer:visible'));
	...
	var saveLandmarkData = function(data) {
		...
		data.pin 			= $('#pins-input .selected .blockmap-pin').data('pin');
	...
	// Load the map
	var adminmap = $('#blockmap-admin-map').blockmap({
		id: getParameter('map'),
		height: 480,
		locations: true,
		sidebar: false,
	...
	$('input[type=submit]').click(function(event) {
		if ($('#blockmap-admin-map').is(':visible')) {
			invalid = false;
			...
			// Trigger event
			$('#blockmap-admin-map').trigger('blockmap-savedata', [newData]);
