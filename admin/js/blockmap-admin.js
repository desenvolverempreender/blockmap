(function($) {
	...
	var BlockMap = function() {
		...
		self.init = function(el, params) {
			...
			self.el = el.addClass('blockmap-element blockmap-loading').height(self.o.height);
		...
		function Sidebar() {
			...
			this.init = function() {
				...
				if (self.data.search) {
					var form = $('<form></form>').addClass('blockmap-search-form').submit(function() {
						return false;
					}).appendTo(this.el);
					self.clear = $('<button></button>').addClass('blockmap-search-clear').click(function() {
						input.val('');
						input.keyup();
					}).appendTo(form);
					var input = $('<input>').attr({'type': 'text', 'spellcheck': 'false', 'placeholder': blockmap_localization.search}).addClass('blockmap-search-input').keyup(function() {
						var keyword = $(this).val();
						s.search(keyword);
					}).prependTo(form);
				}

				this.listContainer = $('<div></div>').addClass('blockmap-list-container').appendTo(this.el);
				this.notfound = $('<p></p>').addClass('blockmap-not-found').text(blockmap_localization.not_found).appendTo(this.listContainer);

				if (!self.data.search) this.listContainer.css('padding-top', '0');
			}

			this.addLevel = function(level) {
				this.list = $('<ol></ol>').text(level.title).addClass('blockmap-list sortable-list').appendTo(this.listContainer);
			}
			...
			this.addCategories = function(categories) {
				...
				$.each(categories, function(index, category) {
					var item = $('<li></li>').addClass('blockmap-list-category').attr('data-category', category.id);
					...
					if (category.icon) $('<img>').attr('src', category.icon).addClass('blockmap-list-thumbnail').prependTo(link);
					$('<span></span>').text('0').addClass('blockmap-list-count').prependTo(link);
					list.append(item);
				});
			}

			this.addLocation = function(data) {
				var item = $('<li></li>').addClass('blockmap-list-location list-item').addClass('blockmap-list-shown').attr('data-location', data.id);
				var link = $('<a></a>').addClass('list-item-handle').attr('href', '#' + data.id).appendTo(item).click(function(e) {
					...
					$('.blockmap-pin[data-location="' + data.id + '"]', self.map).addClass('selected-pin');
				});
				$('<h4></h4>').text(data.title).appendTo(link);
				
				var category = $('.blockmap-list-category[data-category="' + data.category + '"]');

				if (category.length) $('ol', category).append(item);
				else this.list.append(item);

				// Count
				$('.blockmap-list-count', category).text($('.blockmap-list-shown', category).length);
			}
			...
			this.search = function(keyword) {
				...
				$('.blockmap-list li', self.el).each(function() {
					if ($(this).text().search(new RegExp(keyword, "i")) < 0) {
						$(this).removeClass('blockmap-list-shown');
						$(this).slideUp(200);
					} else {
						$(this).addClass('blockmap-list-shown');
						$(this).show();
					}
				});

				$('.blockmap-list > li', self.el).each(function() {
					var count = $('.blockmap-list-shown', this).length;
					$('.blockmap-list-count', this).text(count);
				});

				// Show not-found text
				if ($('.blockmap-list > li.blockmap-list-shown').length > 0) this.notfound.fadeOut(200);
		...
		function ClearButton() {
			...
			this.init = function() {
				this.el = $('<a></a>').attr('href', '#').addClass('blockmap-clear-button').click(function(e) {
					e.preventDefault();
		...
		function ZoomButtons() {
			...
			this.init = function() {
				this.el = $('<div></div>').addClass('blockmap-zoom-buttons').appendTo(self.container);

				this.zoomin = $('<a></ha>').attr('href', '#').addClass('blockmap-zoomin-button').appendTo(this.el);
				...
				this.zoomout = $('<a></ha>').attr('href', '#').addClass('blockmap-zoomout-button').appendTo(this.el);
			...
			this.update = function(scale) {
				...
				else if (scale == self.o.maxscale) this.zoomin.addClass('blockmap-disabled');
			}
		}
		...
		function Styles() {
			...
			this.process = function(styles) {
				...
				// basecolor
				if (self.o.basecolor) css += this.rule('.blockmap-fullscreen, .blockmap-legend', 'background-color', self.o.basecolor);

				// bgcolor
				if (self.o.bgcolor) {
					css += this.rule(
						'.blockmap-button, .blockmap-tooltip-close .blockmap-icon, .blockmap-levels-select, .blockmap-levels button, .blockmap-filter, .blockmap-list-container:before, .blockmap-list-expandable, .blockmap-list-category > a, .blockmap-list-location > a, .blockmap-tooltip-wrap, .blockmap-lightbox, .blockmap-toggle:before',
						'background-color',
						self.o.bgcolor
					);
					css += this.rule('.blockmap-legend-key', 'border-color', self.o.bgcolor);
					css += this.rule('.blockmap-tooltip:after', 'border-color', self.o.bgcolor + ' transparent transparent transparent !important');
					css += this.rule('.blockmap-tooltip-bottom.blockmap-tooltip:after', 'border-color', 'transparent transparent ' + self.o.bgcolor + ' transparent !important');
				}

				// bgcolor2
				if (self.o.bgcolor2) {
					css += this.rule(
						'.blockmap-thumbnail-placeholder, .blockmap-list-location > a:hover, .blockmap-list-location > a:focus, .blockmap-list-location.blockmap-focus > a, .blockmap-list-location.blockmap-active > a, .blockmap-list-category > a:hover, .blockmap-zoom-buttons button.blockmap-disabled, .blockmap-levels button.blockmap-disabled',
						'background-color',
						self.o.bgcolor2
					);
					css += this.rule('a.blockmap-zoomin-button', 'border-color', self.o.bgcolor2);
				}

				// headingcolor
				if (self.o.headingcolor) {
					css += this.rule('.blockmap-search-input, .blockmap-list-category > a, .blockmap-tooltip-title, .blockmap-lightbox-title, .blockmap-list-location h4, .blockmap-element strong, .blockmap-levels-select, .blockmap-list-category h4', 'color', self.o.headingcolor);
					css += this.rule('.blockmap-icon', 'fill', self.o.headingcolor);
				}

				// textcolor
				if (self.o.textcolor) css += this.rule('.blockmap-element, .blockmap-element a, .blockmap-about, .blockmap-list-category > a .blockmap-list-count, .blockmap-search-input::placeholder, .blockmap-lightbox-description', 'color', self.o.textcolor);

				// accentcolor
				if (self.o.accentcolor) css += this.rule('.blockmap-popup-link, .blockmap-accentcolor', 'background-color', self.o.accentcolor);
		...
		function PinDrag() {
			this.init = function() {
				...
				// Pin drag
				$(document).on('mousedown', '.blockmap-element .blockmap-pin', function(event) {
					var pin = $(this);
				...
				// Pin touch
				$(document).on('touchstart', '.blockmap-element .blockmap-pin', function(e) {
					var pin = $(this);
					e.preventDefault();
			...
			this.update = function(location) {
				...
				// Custom fields
				$('#landmark-settings .blockmap-landmark-field').each(function() {
				...
				$('.save-landmark').val(blockmap_localization.save); // button text
		...
		var processData = function(data) {
			...
			self.container = $('<div></div>').addClass('blockmap-container').appendTo(self.el);
			self.map = $('<div></div>').addClass('blockmap-map').appendTo(self.container);
			...
			self.locationid = $('<div>id</div>').addClass('blockmap-locationid').appendTo(self.container);
			...
			if (data.levels) {
				$.each(data.levels, function(index, value) {
					...
					var layer = $('<div></div>').addClass('blockmap-layer').attr('data-floor', value.id).hide().appendTo(self.map);
					switch (extension) {
						...
						case 'svg':							
							...
							$('<div></div>').load(source, function() {
								...
								$(self.o.selector, mapimage).each(function() {
									...
									if (location) {
										//$(this).attr('class', 'blockmap-clickable');
								...
								$(self.o.selector, mapimage).on('click touchend', function() {
									if (!self.dragging) {
										...
										if (location) {
											if ($('#landmark-settings .save-landmark:visible').val() == blockmap_localization.add) $('#landmark-settings .id-input').val(id);
											self.pindrag.update(location);
											$('.selected-pin').removeClass('selected-pin');
											$('.blockmap-pin[data-location="' + id + '"]', self.map).addClass('selected-pin');
										}
					...
					$.each(value.locations, function(index, location) {
						...
						var pin = $('<a></a>').attr('href', '#').addClass('blockmap-pin').css({'top': top + '%', 'left': left + '%'}).appendTo(layer);
			...
			if (self.o.alphabetic) {
				$('.blockmap-list-container ol', self.el).each(function() {
					var mylist = $(this);
					var listitems = mylist.children('.blockmap-list-location').get();
			...
			if (nrlevels > 1) {
				...
				var up = $('<a href="#"></a>').addClass('blockmap-levels-up').appendTo(self.levels);
				self.levelselect.appendTo(self.levels);
				var down = $('<a href="#"></a>').addClass('blockmap-levels-down').appendTo(self.levels);
				...
				up.click(function(e) {
					e.preventDefault();
					if (!$(this).hasClass('blockmap-disabled')) switchLevel('+');
				});
		...
		var addControls = function() {
			var map = self.map,
				mapbody = $('.blockmap-map-image', self.map);
			...
			mapbody.on('mousedown', function(event) {
				...
				map.addClass('blockmap-dragging');
				...
				$(document).on('mouseup', function(event) {
					...
					map.removeClass('blockmap-dragging');
				});
			});

			// Double click
			$(document).on('dblclick', '.blockmap-map-image', function(event) {
				event.preventDefault();
			...
			// Mousewheel
			$('.blockmap-layer', self.el).bind('mousewheel DOMMouseScroll', function(event, delta) {
				event.preventDefault();
		...
		var switchLevel = function(target) {
			...
			shownLevel = target;
			var layer = $('.blockmap-layer[data-floor="' + target + '"]', self.map);

			// Target layer is active
			if (layer.is(':visible')) return;

			// Show target layer
			$('.blockmap-layer:visible', self.map).hide();
			layer.show();

			// Update control
			var index = self.levelselect.get(0).selectedIndex,
				up = $('.blockmap-levels-up', self.levels),
				down = $('.blockmap-levels-down', self.levels);

			up.removeClass('blockmap-disabled');
			down.removeClass('blockmap-disabled');
			if (index == 0) {
				up.addClass('blockmap-disabled');
			}
			else if (index == self.levelselect.get(0).length - 1) {
				down.addClass('blockmap-disabled');
			}
		}
	...
	$.fn.blockmap = function(params) {
		var len = this.length;

		return this.each(function(index) {
			var me = $(this),
				key = 'blockmap' + (len > 1 ? '-' + ++index : ''),
				instance = (new BlockMap).init(me, params);

			me.data('blockmap', instance);
		});
	};
})(jQuery);