...
function blockmap_add_example_maps() {
...
// New map type select
function blockmap_new_map_type() {
	// Built-in maps
	$maps = array(
		'world-us'		=> __('World + US', 'blockmap'),
		'canada' 		=> __('Canada', 'blockmap'),
		'australia' 	=> __('Australia', 'blockmap'),
		'france' 		=> __('France', 'blockmap'),
		'germany' 		=> __('Germany', 'blockmap'),
		'uk' 			=> __('United Kingdom', 'blockmap'),
		'italy' 		=> __('Italy', 'blockmap'),
		'netherlands' 	=> __('Netherlands', 'blockmap'),
		'switzerland' 	=> __('Switzerland', 'blockmap'),
		'russia' 		=> __('Russia', 'blockmap'),
		'china' 		=> __('China', 'blockmap'),
		'brazil' 		=> __('Brazil', 'blockmap')
	);
	?>
	<h3><?php _e('Select Map Type', 'blockmap'); ?></h3>
	<p><?php _e('Create a custom map using your own file(s) or select one of the built-in maps.', 'blockmap'); ?></p>
	<select id="blockmap-new-type" name="new-map-type">
		<option value="custom"><?php _e('Custom', 'blockmap'); ?></option>
		<option value="world"><?php _e('World', 'blockmap'); ?></option>
		<option value="continents"><?php _e('Continents', 'blockmap'); ?></option>
		<option value="usa"><?php _e('USA', 'blockmap'); ?></option>
	<?php foreach ($maps as $key => $map) : ?>
		<option disabled value="<?php echo $key; ?>"><?php echo $map; ?></option>
	<?php endforeach; ?>
	</select>
	<br>
	<p><a href="edit.php?post_type=blockmap_map&page=upgrade_blockmap"><?php _e('Upgrade BlockMap', 'blockmap'); ?></a> for more built-in Maps</p>
	<br>
	<div id="blockmap-mapfile">
		<h3><?php _e('Define Map File', 'blockmap'); ?></h3>
		<p><?php _e('Upload or select map file from library. SVG, JPG and PNG formats supported.', 'blockmap'); ?></p>
		<label class="field-small">
			<b><?php _e('Name', 'blockmap'); ?></b><br>
			<input type="text" name="new-map-name" class="input-text title-input" value="My Map">
		</label>
		<label class="field-small">
			<b><?php _e('ID (required)', 'blockmap'); ?></b><br>
			<input type="text" name="new-map-id" class="input-text id-input" value="my-map">
		</label>

		<div class="field-medium">
			<label><b><?php _e('Map File (required)', 'blockmap'); ?></b><br>
		...
		<label class="field-small">
			<b><?php _e('Width (reqiured)', 'blockmap'); ?></b><br>
			<input type="text" name="new-mapwidth" class="input-text" value="">
		</label>
		<label class="field-small">
			<b><?php _e('Height (reqiured)', 'blockmap'); ?></b><br>
...
// Built-in Maps
function blockmap_map_type($map) {
	$mapdir = plugins_url('../maps', __FILE__);
	$maps = array(
